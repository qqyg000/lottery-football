...
- FotMob 各赛事赛季接口、阿塞拜疆超级联赛、韩国职业联赛、韩国杯、苏足总杯、芬兰超级联赛，以及上述来源未覆盖且不重复的竞彩场次
...
- Futbol24 `league_id=520`：苏足总杯完整赛季历史和近期比赛，作为 FotMob 的交叉校验和更新补充源
...
基础导入脚本只保留 18 类赛事、有完场比分且至少包含一组完整初盘赔率的记录。官方体彩增量脚本还会追加当前在售但本地尚无赛程行的比赛；未完赛行保留空比分并按 `SCHEDULED` 加载，不会误标为已完赛。接口英文球队名与赔率表英文名关联后，统一转换为赔率表中的最新中文名。脚本会继续执行 `reconcile-historical-scores.mjs`，用 FotMob 或已核验接口赛果纠正原始文件中合计 7 球的压缩比分、跨日日期和主客顺序，再重建去重后的历史比赛文件。
...
指定日期范围的官方赔率可以先通过接口写入缓存，再用脚本合并到内置文件。历史补取接口只保留系统支持的 18 类赛事，并从 `hadList`、`hhadList` 中选择更新时间最早的有效记录作为初盘：
