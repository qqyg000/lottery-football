...
const historicalRows = originalHistoricalRows.filter(row => (
  row.match_date >= MINIMUM_HISTORY_DATE && !isExcludedHistoricalRow(row)
))
const oddsRows = parseCsv(historicalOddsText).filter(row => (
  !EXCLUDED_COMPETITIONS.has(String(row.competition ?? '').trim().toUpperCase())
))
